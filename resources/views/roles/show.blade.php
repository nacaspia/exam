                    ...
                    <div class="panel-body">
                        <div class="row g-3">
                            <div class="col-xxl-3 col-lg-4 col-sm-6">
                                ...
                                <input type="text" class="form-control form-control-sm" disabled="disabled" name="name" value="{{ old('name', $role['name'] ?? '') }}">
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            @foreach($permissionLabels as $label)
                                <div class="cat-group">
                                    <div class="form-check">
                                        <input class="form-check-input has-sub"
                                               type="checkbox" disabled="disabled"
                                               id="label{{$label['id']}}"
                                            {{ collect($label['permissions'])->pluck('id')->intersect($selectedPermissions)->count() == count($label['permissions']) ? 'checked' : '' }}>
                                    ...
                                    <div class="sub-cat-group" >
                                        @foreach($label['permissions'] as $permission)
                                            <div class="form-check">
                                                <input class="form-check-input" disabled="disabled" name="permissions[]"  {{ in_array($permission['id'], old('permissions', $selectedPermissions)) ? 'checked' : '' }} value="{{ $permission['id'] }}" type="checkbox" id="cat{{$permission['permission_label_id']}}">
