(function($) {
    ...
    $(document).ready(function() {
        ...
        // İlk öncə bütün sub-cat-group gizlədilir
        $('.sub-cat-group').hide();

        // '+' iconuna klik
        $('.cat-group .form-check-label i').on('click', function(e){
            e.preventDefault(); // parent label klikini bloklayır
            e.stopPropagation();

            let $parentGroup = $(this).closest('.cat-group');
            let $subGroup = $parentGroup.find('.sub-cat-group');
        ...
        // Page load zamanı seçilmiş permission varsa açıq göstər
        $('.cat-group').each(function(){
            if($(this).find('.sub-cat-group .form-check-input:checked').length > 0){
                $(this).find('.sub-cat-group').show();
                $(this).find('.form-check-label i').removeClass('fa-plus').addClass('fa-minus');
            }
        });
      /*  $('.sub-cat-group').hide();
